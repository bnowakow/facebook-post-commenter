        ...
        if (beginingOfNextPostLocation > -1) {
            ...
            while (true) {
                ...
                if (commentTextBoxPosition > -1) {
                    // can be commented
                    logger.debug("\t\tpost ${commentNumber} by ${commentAuthor} can be commented")
                    ...
                    if (adminUsernamePosition == -1) {
                        ...
                        logger.debug("\t\t\tpost ${commentNumber} by ${commentAuthor} doesn't contain admin response")
...
//                        .sendKeys(replyMessage.replace("\n", Keys.chord(Keys.SHIFT, Keys.ENTER)))
                            ...
                            logger.error("NoSuchElementException exception has been thrown during processing of ${id} post on ${commentNumber}th post by ${commentAuthor}")
                        } catch (e: Exception) {
                            logger.error(e.message)
                            logger.error("Exception exception has been thrown during processing of ${id} post on ${commentNumber}th post by ${commentAuthor}")
                        }
                        ...
                        logger.debug("\t\t\tpost ${commentNumber} by ${commentAuthor} contains admin response")
                    ...
                    logger.debug("\t\tpost ${commentNumber} by ${commentAuthor} can't be commented")
                    // TODO this breaks if there's longer post (i.e. with comments)
                    // TODO example is birth post no. 85
                    for (i in 1..8) {
                        driver.findElement(By.cssSelector("body")).sendKeys(Keys.TAB)
